...
function DesignCanvas({ children, minScale, maxScale, style, columns, compact }) {
  ...
  return (
    <DCCtx.Provider value={api}>
      <DCViewport minScale={minScale} maxScale={maxScale} columns={columns} compact={compact} style={style}>{ready && children}</DCViewport>
...
// ─────────────────────────────────────────────────────────────
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {}, columns, compact }) {
    ...
    >
      <div
        ...
        style={{
          ...
          minHeight: '100%',
          padding: compact ? '24px 16px 40px' : '60px 0 80px',
          ...(columns ? { display: 'grid', gridTemplateColumns: `repeat(${columns}, max-content)`, columnGap: 36, alignItems: 'start' } : null),
        }}
      >
...
// ─────────────────────────────────────────────────────────────
function DCSection({ id, title, subtitle, children, gap }) {
  const ctx = React.useContext(DCCtx);
  const cmp = !!(ctx && ctx.compact);
  if (gap == null) gap = cmp ? 20 : 48;
  const sid = id ?? title;
  ...
  return (
    <div data-dc-section={sid}
      style={{ marginBottom: cmp ? 'calc(26px * var(--dc-inv-zoom, 1))' : 'calc(80px * var(--dc-inv-zoom, 1))', position: 'relative' }}>
      {!cmp && (
      ...
      <div style={{ display: 'flex', flexWrap: 'nowrap', ...(cmp ? { columnGap: 24 } : { gap }), padding: cmp ? '0 16px' : '0 60px', alignItems: 'flex-start', width: 'max-content' }}>
        {order.map((k) => (
